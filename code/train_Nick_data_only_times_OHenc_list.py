...
import sys, os
if len(sys.argv)<2:
    sys.exit("python train.py eventlog list_architecture ")
...
architecture = [int(i) for i in sys.argv[2].split(' ')]
print(architecture)
#LR=float(sys.argv[4])
...
#l1 = LSTM(architecture[0], consume_less='gpu', init='glorot_uniform', return_sequences=True, dropout_W=0.2)(main_input) # the shared layer
#b1 = BatchNormalization()(l1)
l_hidden_bn_1=[]
l_hidden_1=[]
#l_hidden_1.append(main_input) # the layer specialized in activity prediction
l_hidden_bn_1.append(main_input)

for layer in range(0,len(architecture)-1):
    l_hidden_1.append(LSTM(architecture[layer], consume_less='gpu', init='glorot_uniform', return_sequences=True, dropout_W=0.2)(l_hidden_bn_1[layer])) # the layer specialized in activity prediction
...
with open("output_files/models/"+eventlog+"_"+str(".".join(str(s) for s in architecture))+"_model.json", "w") as json_file:
...
model_checkpoint = ModelCheckpoint('output_files/models/'+eventlog+"_"+str(".".join(str(s) for s in architecture))+'_weights_best.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
